...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--baseline',required=True); p.add_argument('--finetuned',required=True); p.add_argument('--output-dir',required=True); p.add_argument('--bootstrap',type=int,default=10000); p.add_argument('--seed',type=int,default=42); p.add_argument('--exclude-ids',help='JSON file containing an exclusions list with sample_id fields')
    a=p.parse_args(); base=read(a.baseline); fine=read(a.finetuned); out=Path(a.output_dir); out.mkdir(parents=True,exist_ok=True)
    exclusions=[]
    if a.exclude_ids:
        exclusion_doc=json.loads(Path(a.exclude_ids).read_text(encoding='utf-8'))
        exclusions=exclusion_doc.get('exclusions',[])
        excluded_ids={x['sample_id'] for x in exclusions}
        observed_ids={x['id'] for x in base}|{x['id'] for x in fine}
        missing=excluded_ids-observed_ids
        if missing: raise SystemExit(f'Excluded IDs not found in predictions: {sorted(missing)}')
        base=[x for x in base if x['id'] not in excluded_ids]
        fine=[x for x in fine if x['id'] not in excluded_ids]
    ...
    result={'paired_samples':len(ids),'songs':len(songs),'excluded_samples':len(exclusions),'exclusions':exclusions,'baseline':aggregate(base),'finetuned':aggregate(fine),
